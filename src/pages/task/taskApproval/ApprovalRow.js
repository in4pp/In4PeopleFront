...
function ApprovalRow({ approval : { docCode, approvalMem, docType, isApproved, title,
    bookmark, docAttachmentList, approverList, reportDate }, memCode }) {
    ...
    const onClickBookmarkHandler = () => { 
        ...
        bookmark == undefined ? dispatch(callPostBookmarkAPI({form : form})) : dispatch(callDeleteBookmarkAPI({ form : form }));
        }
    ...
    return (
        <>
            <tr>
                ...
                <td><button onClick={ onClickBookmarkHandler }>{ bookmark == undefined ? <PlainStar /> : <BlueStar /> }</button></td>
                <td>{ docType }</td>
                <td>{ docCode }</td>
                <td>{ title }</td>
                ...
                <td>0개</td>
                <td>{reportDate.substring(0,10)}</td>
            </tr>
        </>
    )
}
